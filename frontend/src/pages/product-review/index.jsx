...
const ProductReview = () => {
    ...
    const getReviews = async (reviewValues) => {
      try {
        ...
        const newStats = {
          ...
          modeReviewValue: values.reduce((currentMax, value, index, arr) => {
          ...
          stdDevReviewValue: Math.sqrt(values.reduce((sum, value) => sum + Math.pow(value - meanReviewValue, 2), 0) / values.length),
  ...
  const handleRatingChange = (event) => {
    ...
    setHighlightedStars(
      highlightedStars.map((_, index) => index < selectedStars)
    );
  };
  ...
  const handleStarClick = (index) => {
    ...
    setReviewValue(selectedStars);
    setHighlightedStars(
      highlightedStars.map((_, starIndex) => starIndex < selectedStars)
    );
  };
